...
export default function SettingsPage({ navigate }: Props) {
  ...
  useEffect(() => {
    api.users.get(api.ME_ID).then(u => {
      if (u && u.name) {
        setName(u.name);
  ...
  const saveProfile = async () => {
    setSaving(true);
    await api.users.updateProfile(name, statusText, bio);
    setSaving(false);
    setSaved(true);
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: () => void }) => (
    <button onClick={onChange} className={`w-12 h-6 rounded-full transition-all relative ${value ? "grad-btn" : "bg-white/10"}`}>
    ...
    <div className="p-6 max-w-2xl mx-auto animate-fade-in">
      ...
      <div className="glass rounded-2xl p-6 mb-6 border border-white/8 flex items-center gap-5">
        <div className="relative cursor-pointer group">
          <div className="avatar-ring w-20 h-20 animate-pulse-glow">
            <div className="w-full h-full rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-3xl">😊</div>
          </div>
        ...
        <div>
          <h3 className="font-semibold text-white text-lg">{name || "Загрузка..."}</h3>
      ...
      <div className="glass rounded-2xl p-5 mb-4 border border-white/8">
        <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
          <Icon name="User" size={16} className="text-purple-400" />Профиль
        </h3>
        <div className="flex flex-col gap-3">
          <div>
            <label className="text-xs text-white/40 mb-1.5 block">Имя</label>
            <input className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-purple-500/50 transition-colors"
          ...
          <div>
            ...
            <textarea className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-purple-500/50 transition-colors resize-none"
              rows={2} value={bio} onChange={e => setBio(e.target.value)} />
          </div>
        </div>
        <button onClick={saveProfile} disabled={saving} className="grad-btn px-5 py-2.5 rounded-xl text-sm font-semibold mt-4 flex items-center gap-2 disabled:opacity-70">
          {saved ? <><Icon name="Check" size={15} />Сохранено!</> : saving ? "Сохранение..." : "Сохранить"}
        </button>
      </div>

      <div className="glass rounded-2xl p-5 mb-4 border border-white/8">
        <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
          <Icon name="Bell" size={16} className="text-pink-400" />Уведомления
        </h3>
      ...
      <div className="glass rounded-2xl p-5 mb-4 border border-purple-500/20 cursor-pointer hover:border-purple-500/40 transition-colors"
        style={{ background: "var(--grad-card)" }} onClick={() => navigate("invites")}>
